...
function submit_btn_Click {
    ...
    if ($DatePicker_end_edit_lbl.text -eq $DISPLAY_NULL){
        ...
        if ($true) {
            ...
            $job_Search_path_Stop = "JD:\\!!!DAVE_DEMO\Stop_" + $Name
            $job_Search_path_Start = "JD:\\!!!DAVE_DEMO\Start_" + $Name
            ...
            #$job = New-Item JD:\!!!DAVE_DEMO\MyJob1234 -itemtype Job -MethodName PowerShell
            ...
            ###
            #$JAMS_Name_Begin = MVPSI.JAMS.Submit.Load(out si, "Start_")
